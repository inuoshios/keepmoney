...
router
  ...
  .get(requireUser, async (req: Request, res: Response) => {
    // @ts-ignore
    const { id } = req.user;
...
router
  ...
  .get(
    requireUser,
    validate(getSingleBudgetSchema),
  ...
  .patch(
    ...
    async (
      req: Request<
        updateBudgetParams['params'],
        {},
        updateBudgetParams['body']
      >,
      res: Response
    ) => {
      const { budgetId } = req.params;
      ...
      const newBudget = await updateBudget(
        { budgetId },
        { ...req.body },
      ...
      res
        ...
        .json({ msg: 'budget updated successfully', budget: newBudget });
    }
  )
  .delete(
    requireUser,
